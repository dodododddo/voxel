...
class Voxelmap(object):
    def __init__(self,ppi:int = 2,size:vec3i = vec3i(1500,250,2500)): # size默认值为rmuc场地全图尺寸，单位为cm；ppi默认值为2cm
        ...
        self.probability_grid = ti.field(ti.f32) # 体素地图的叶结点存放该体素被占据的logit变换后的概率：y = log(x/1-x)
        ...
        self.grid.place(self.probability_grid)
        assert(self.probability_grid.shape[0] * self.ppi >= self.size[0] \
               and self.probability_grid.shape[1] * self.ppi >= self.size[1] \
               and self.probability_grid.shape[2] * self.ppi >= self.size[2]) 
        # 确保体素地图能覆盖预设范围
    ...
    def add_point(self,point:vec3i)-> bool: # 逐点更新
        ...
        if success_add:
            ...
            self.probability_grid[grid_increase] += 0.6  # 该点对应占据体素位置占据概率对数值上升

            regu = ti.max(ti.abs(point[0]),-1 * point[1],point[2]) 
            grad = point / regu      # 找到x,y,z轴下降最缓的方向，保证沿此方向每走一步只经过一个体素
            for i in range(regu):
                grid_reduce = self.Pos2Grid(point - grad * (i + 1)) # 沿此方向下降搜索可能遮挡该点的体素，直到到达原点附近体素
                if ti.is_active(self.grid,grid_reduce):
                    self.probability_grid[grid_reduce] -= 0.05 # 未被激活的体素不考虑，已被激活的体素认为被占据的概率下降
    ...
    def visit_grid(self,grid:vec3i)-> ti.f32:
        ...
        if ti.is_active(self.grid,[grid[0],grid[1],grid[2]]):
            probablity =  1 - 1 / (1 + ti.math.exp(self.probability_grid[grid[0],grid[1],grid[2]]))
        return probablity
